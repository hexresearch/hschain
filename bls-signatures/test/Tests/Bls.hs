...
testBls = testGroup "Private keys" [
    testCase "Signing and serializing" $ do
        ...
        (hexify $ serializeInsecureSignature sig1) @?=
        ...
        (hexify $ serializeInsecureSignature sig2) @?=
    ...
    , testCase "Deserialization" $ do
        ...
        getFingerprint pk1 @?= 0x26d53247
        getFingerprint pk2 @?= 0x289bb56e
        (hexify $ serializePublicKey $ publicKeyFromBytes $ serializePublicKey pk1) @?=
            "02a8d2aaa6a5e2e08d4b8d406aaf0121a2fc2088ed12431e6b0663028da9ac5922c9ea91cde7dd74b7d795580acc7a61"
        (hexify $ serializePublicKey $ publicKeyFromBytes $ serializePublicKey pk2) @?=
            "83fbcbbfa6b7a5a0e707efaa9e6de258a79a59116dd889ce74f1ab7f54c9b7ba15439dcb4acfbdd8bcffdd8825795b90"
        (hexify $ serializeInsecureSignature $ insecureSignatureFromBytes $ serializeInsecureSignature sig1) @?=
            "93eb2e1cb5efcfb31f2c08b235e8203a67265bc6a13d9f0ab77727293b74a357ff0459ac210dc851fcb8a60cb7d393a4\
            \19915cfcf83908ddbeac32039aaa3e8fea82efcb3ba4f740f20c76df5e97109b57370ae32d9b70d256a98942e5806065"
        (hexify $ serializeInsecureSignature $ insecureSignatureFromBytes $serializeInsecureSignature sig2) @?=
            "975b5daa64b915be19b5ac6d47bc1c2fc832d2fb8ca3e95c4805d8216f95cf2bdbb36cc23645f52040e381550727db42\
            \0b523b57d494959e0e8c0c6060c46cf173872897f14d43b2ac2aec52fc7b46c02c5699ff7a10beba24d3ced4e89c821e"
    , testCase "Equivalence" $ do
        let message = "Let's sign me!"
            sk1 = fromSeed "\10\20\30\40\50"
            sk2 = fromSeed "\60\70\80\90\10"
            pk1 = getPublicKey sk1
            pk2 = getPublicKey sk2
        ...
        (      sk1  `privateKeyEq` (privateKeyFromBytes $ serializePrivateKey sk1)) @? "Private keys must be equals"
        (      sk2  `privateKeyEq` (privateKeyFromBytes $ serializePrivateKey sk2)) @? "Private keys must be equals"
        ...
        (      sig1 `insecureSignatureEq` (insecureSignatureFromBytes $ serializeInsecureSignature sig1)) @? "Signatures must be equals"
        (      sig2 `insecureSignatureEq` (insecureSignatureFromBytes $ serializeInsecureSignature sig2)) @? "Signatures must be equals"
        ...
        (      pk1 `publicKeyEq` (publicKeyFromBytes $ serializePublicKey pk1)) @? "Public keys must be equals"
        (      pk2 `publicKeyEq` (publicKeyFromBytes $ serializePublicKey pk2)) @? "Public keys must be equals"
    -- *** ---
    , testCase "Hashing" $ do
    ...
    , testCase "Aggregations" $ do
        let message = "My test message"
            ...
            sig1 = signInsecure sk1 message
            sig2 = signInsecure sk2 message
        insecureVerify sig1 [msgHash] [getPublicKey sk1] @? "Must be verifiable with sig1"
        insecureVerify sig2 [msgHash] [getPublicKey sk2] @? "Must be verifiable with sig2"
        let sigs = [sig1, sig2]
            pks =  [getPublicKey sk1, getPublicKey sk2]
            aggSig = aggregateInsecureSignatures sigs
            aggPk  = aggregateInsecurePublicKeys pks
        insecureVerify aggSig [msgHash] [aggPk] @? "Must be verifialbe with aggregated signatures"
    ]
