...
testBls = testGroup "Private keys" [
    ...
    , testCase "Deserialization" $ do
        let (Just sk1) = privateKeyDeserialize (dehexify "022fb42c08c12de3a6af053880199806532e79515f94e83461612101f9412f9e")
            (Just sk2) = privateKeyDeserialize (dehexify "502c5661f5af46ed48ddc3b5332e21b93cc7d0a84df46c4b9c7fe8f25ef48d66")
            message1 = "\7\8\9"
        ...
        publicKeyGetFingerprint pk1 @?= 0x26d53247
        publicKeyGetFingerprint pk2 @?= 0x289bb56e
        (hexify $ publicKeySerialize $ fromJust $ publicKeyDeserialize $ publicKeySerialize pk1) @?=
            "02a8d2aaa6a5e2e08d4b8d406aaf0121a2fc2088ed12431e6b0663028da9ac5922c9ea91cde7dd74b7d795580acc7a61"
        (hexify $ publicKeySerialize $ fromJust $ publicKeyDeserialize $ publicKeySerialize pk2) @?=
            "83fbcbbfa6b7a5a0e707efaa9e6de258a79a59116dd889ce74f1ab7f54c9b7ba15439dcb4acfbdd8bcffdd8825795b90"
        (hexify $ insecureSignatureSerialize $ fromJust $ insecureSignatureDeserialize $ insecureSignatureSerialize sig1) @?=
            "93eb2e1cb5efcfb31f2c08b235e8203a67265bc6a13d9f0ab77727293b74a357ff0459ac210dc851fcb8a60cb7d393a4\
            \19915cfcf83908ddbeac32039aaa3e8fea82efcb3ba4f740f20c76df5e97109b57370ae32d9b70d256a98942e5806065"
        (hexify $ insecureSignatureSerialize $ fromJust $ insecureSignatureDeserialize $insecureSignatureSerialize sig2) @?=
            "975b5daa64b915be19b5ac6d47bc1c2fc832d2fb8ca3e95c4805d8216f95cf2bdbb36cc23645f52040e381550727db42\
            \0b523b57d494959e0e8c0c6060c46cf173872897f14d43b2ac2aec52fc7b46c02c5699ff7a10beba24d3ced4e89c821e"
    , testCase "Equivalence" $ do
        ...
        (      sk1  `privateKeyEq` (fromJust $ privateKeyDeserialize $ privateKeySerialize sk1)) @? "Private keys must be equals"
        (      sk2  `privateKeyEq` (fromJust $ privateKeyDeserialize $ privateKeySerialize sk2)) @? "Private keys must be equals"
        ...
        (      sig1 `insecureSignatureEq` (fromJust $ insecureSignatureDeserialize $ insecureSignatureSerialize sig1)) @? "Signatures must be equals"
        (      sig2 `insecureSignatureEq` (fromJust $ insecureSignatureDeserialize $ insecureSignatureSerialize sig2)) @? "Signatures must be equals"
        ...
        (      pk1 `publicKeyEq` (fromJust $ publicKeyDeserialize $ publicKeySerialize pk1)) @? "Public keys must be equals"
        (      pk2 `publicKeyEq` (fromJust $ publicKeyDeserialize $ publicKeySerialize pk2)) @? "Public keys must be equals"
    -- *** ---
    , testCase "Hashing" $ do
        (hexify $ hash256serialize $ hash256 "\1\2\3") @?= "039058c6f2c0cb492c533b0a4d14ef77cc0f78abccced5287d84a1a2011cfb81"
    -- *** ---
