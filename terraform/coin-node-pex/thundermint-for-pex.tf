...
resource "docker_container" "node" {
  ...
  env   = [ "THUNDERMINT_NODE_SPEC={ \"nspecPrivKey\":\"${var.private_keys[count.index]}\", \"nspecDbName\": \"db/node-${count.index+1}\", \"nspecLogFile\" : [{ \"type\": \"ScribeTXT\", \"path\" : \"logs/node-${count.index+1}\", \"severity\" : \"Debug\", \"verbosity\" : \"V2\" }], \"nspecWalletKeys\"  : [${count.index*1},1]}"
          , "THUNDERMINT_KEYS=${jsonencode(values(var.private_keys))}"
          ]
  command = ["/bin/thundermint-coin-node", "--max-h", "20", "--prefix", "/thundermint", "--delay", "100", "--check-consensus", "--deposit", "1000", "--keys", "2000" ]
  #ports = {
  #  ip = "172.17.0.1"
  #  internal = "${count.index + 60001}"
  #  external = "${count.index + 60001}"
  #}
  volumes = {
...
resource "null_resource" "configure-nodes-ips" {
  ...
  provisioner "local-exec" {
    command = "bootstrap -t ring  -l '${jsonencode(docker_container.node.*.ip_address)}' -n ${element(docker_container.node.*.ip_address, count.index)} -p 49999"
  }
}
